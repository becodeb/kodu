...
- [x] 8.1 `prisma/migrations/20260923000000_atribucion_admin/migration.sql`: `Project.lastAdminActorId` (FK, `onDelete: SetNull`) + `lastAdminActionAt DateTime?`; `ChatMessage.authorUserId` (FK, `onDelete: SetNull`). Apply; `prisma migrate resolve --applied`.
- [x] 8.2 `src/lib/projects.ts`: add `interface Actor { id: string; role: 'DOCENTE' | 'ADMIN' }`; `findProjectForActor(projectId, actor)` (admin bypasses `userId` filter); `findWorkspaceProjectForActor(projectId, actor)` (same bypass + threads/attachments in one query). **Delete `findOwnedProject` and `assertOwnedProject`** — the latter is referenced by nothing outside its own file.
- [x] 8.3 Switch all call sites to the actor helper: `uploads/index.ts:31`, `chat/cancel.ts:30`, `chat/stream.ts:270`, `projects/[id].ts:22,55`, `projects/[id]/screenshot.ts:18,42`, `projects/[id]/threads.ts:19,44` — 9 real call sites, not 7 (see Deviations). Refusal copy stays `"El recurso no existe o no es tuyo."` for a docente.
- [x] 8.4 **Trap respected — `src/pages/app/index.astro:10` untouched** except for adding `lastAdminActionAt` to the `select` (needed for 8.7's card badge; the `where: { userId: user.id }` ownership-listing clause itself is unchanged).
- [x] 8.5 `src/pages/app/project/[id].astro:11`: swapped to `findWorkspaceProjectForActor`; when `actor.role === 'ADMIN' && actor.id !== project.userId`, writes `lastAdminActorId`/`lastAdminActionAt` on `Project` (via `marcarSiActuaAdmin`, called from `chat/stream.ts` and `projects/[id].ts` PATCH — the two actual mutation surfaces) and `authorUserId` on the docente-role `ChatMessage` of that turn (never the assistant reply).
- [x] 8.6 Created `src/components/admin/BannerAdmin.astro`: full-width `role="status"` strip in document flow under the breadcrumb, `border-brand-300 bg-brand-50 text-brand-700` (dark tokens redefine automatically), names the owner, `Volver al panel →` link to `/admin/usuarios/{ownerId}` — not a warning colour.
- [x] 8.7 `src/components/workspace/ChatPanel.tsx`: bubbles with `authorName` set (server-resolved from `authorUserId`) show `{Nombre} (administración)` above them; `src/pages/app/index.astro`'s project card shows `Editado por administración · {haceTiempo(...)}` while `lastAdminActionAt` is under a week old.
- [x] 8.8 DB-state check embedded in `e2e/m8-proyectos-ajenos.ts`: an admin fetches a non-owned project (direct `prisma.project`/`chatMessage` reads confirm `lastAdminActorId`/`authorUserId` populated, and the assistant reply's `authorUserId` stays `null`); the docente-refusal battery confirms no ownership row ever links the rejected actor to the project.
- [x] 8.9 Browser check `e2e/m8-proyectos-ajenos.ts` (named per the orchestrator's explicit instruction, not `m8-acceso-cruzado.ts` — see Deviations), both themes: admin opens a non-owned project, sends a prompt via a local SSE mock, a resource generates as it would for the owner (real `update_resource_code` tool call, not just text); banner renders naming the owner; owner later opens the project and sees the attribution mark, both in the chat bubble and on the `/app` card.
- [x] 8.10 `npm run check` passes (exit 0, no output).
- [x] 8.11 **M8 checkpoint**: deliverable.
